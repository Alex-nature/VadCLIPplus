...
def train(model, train_loader, test_loader, args, label_map: dict, device):
    ...
    # 设置日志记录器
    log_filename = f"logs/xd_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    logging.basicConfig(
        level=logging.INFO,
    ...
    for e in range(epoch, args.max_epoch):
        ...
        total_loss = avg_loss1 + avg_loss2 + avg_loss3 * 1e-4
        ...
        logger.info(f"验证结果 - AUC: {AUC:.4f}, AP: {AP:.4f}, mAP: {mAP:.4f}")
    ...
    # 保存训练历史到JSON文件
    history_filename = f"logs/xd_training_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
